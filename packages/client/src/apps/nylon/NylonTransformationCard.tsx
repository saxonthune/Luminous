  ...
  return (
    <div style={{ width: '100%', height: '100%', display: 'flex', 'flex-direction': 'column',
      ...
      <Show when={!props.overview} fallback={
        <strong title={props.item.name} style={{ 'font-size': '40px', 'line-height': '1.1', margin: 'auto 0',
          'overflow-wrap': 'anywhere', overflow: 'hidden', display: '-webkit-box', '-webkit-line-clamp': 5,
          '-webkit-box-orient': 'vertical' }}>{props.item.name}</strong>
      }>
      ...
      <strong title={props.item.name} style={{ 'font-size': '24px', 'line-height': '1.15', display: '-webkit-box',
        ...
        <svg aria-label={`Contents schematic of ${props.item.name}`} role="img"
          ...
          <For each={preview().edges}>{(edge) => {
            ...
            return <line x1={source().x + source().w} y1={source().y + source().h / 2}
              ...
              stroke={`var(--${edge.styling?.colorToken ?? 'fg-muted'})`} stroke-width="1.5"
              stroke-dasharray={edge.styling?.dash === 'dashed' ? '4 2' : undefined}
      ...
      <Show when={hasContents()}>
        <div style={{ 'margin-top': 'auto', display: 'flex', 'align-items': 'center', gap: '6px',
          ...
          <Show when={props.onOpenPip}>
            <button type="button" aria-label={`Open PIP of ${props.item.name}`} title="Open contents beside this view"
              on:pointerdown={(event) => event.stopPropagation()}
              on:click={(event) => { event.stopPropagation(); props.onOpenPip?.(props.item.id); }}
              style={{ padding: '4px 6px', border: '1px solid var(--border-strong)', 'border-radius': '4px', color: 'var(--fg)' }}>PIP</button>
          </Show>
