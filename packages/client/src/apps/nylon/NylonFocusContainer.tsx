...
export function NylonFocusContainer(props: { name: string; selected: boolean; overview?: boolean }) {
  return <div data-nylon-focus-container aria-label={`${props.name} container`}
    ...
    <strong title={props.name} style={{ 'font-size': props.overview ? '34px' : '24px', overflow: 'hidden',
      'text-overflow': 'ellipsis', 'white-space': 'nowrap' }}>{props.name}</strong>
    <span hidden={props.overview} style={{ 'font-size': '12px', color: 'var(--fg-muted)', 'white-space': 'nowrap' }}>Children</span>
  </div>;
}
