...
The render tree (component A renders component B) is expressed entirely through `parentId` nesting. Render edges would be redundant — the containment IS the relationship. This keeps the edge set focused on cross-boundary flow: reactive dependencies and external data.
