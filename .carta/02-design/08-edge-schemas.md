...
deps: [doc03.03, doc02.05.01, doc02.06.02, doc02.07]
---
...
- `kind: 'edge'` — required discriminant distinguishing this from `NodeSchema`. See doc02.06.02 for the ADR.
...
See doc02.05.01 for the full data contract cactus reads, and doc02.05.03 for the layout algorithms.
...
All four carry `kind: 'edge'` and `directed: true`. None set `layoutRole: 'tree'` — they are semantic annotations on the graph, not layout drivers. See doc02.07 for the pipeline's classification rules.
