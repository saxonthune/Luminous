...
| doc02.21 | Dataflow document format | doc | The *.dataflow.json document — Boxes and Flows carrying design intention; TypeScript types own the shape, a sidecar JSON Schema describes it | dataflow, format, contract, schema |
| doc02.22 | Dataflow operations | doc | The operation set over a dataflow Document — the dataflow MCP tool group's verbs, their validation rules, and the check warnings | dataflow, mcp, operations, tools |
| doc02.23 | Dataflow Designer app | doc | The client app — read-only projection of dataflow Documents onto cactus, live-reloading as the agent writes; document plumbing shared with Luminous Canvas | dataflow, apps, client, rendering |

Topics: actions, ai, animation, api, app-shell, apps, architecture, boundaries, boundary, cactus, cactus-boundary, canvas, canvas-engine, chrome, client, co-location, component-tree, components, concepts, contract, crystallization, data, dataflow, decoration, derivation, design, disclosure, edges, examples, formalization, format, graph, http, identity, iteration, jackson, mcp, menus, milestone-1, node, operations, pack, packs, pdr, pipeline, primitives, projection, property-graph, reference, renderer, rendering, schema, schemas, server, shell, solid, statechart, static-analysis, tools, ui, unfolding, view, views
