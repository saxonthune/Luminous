...
const componentSchema: Schema = { name: 'component', label: 'Component', primitives: [...ENTITY_PRIMITIVES], legendDescription: 'Solid.js component function that returns JSX' };
const hookSchema: Schema      = { name: 'hook',      label: 'Hook',      primitives: [...ENTITY_PRIMITIVES], legendDescription: 'Custom hook that creates or composes reactive state' };
const signalSchema: Schema    = { name: 'signal',    label: 'Signal',    primitives: [...ENTITY_PRIMITIVES], legendDescription: 'Reactive signal (createSignal)' };
const storeSchema: Schema     = { name: 'store',     label: 'Store',     primitives: [...ENTITY_PRIMITIVES], legendDescription: 'Reactive store (createStore)' };
const memoSchema: Schema      = { name: 'memo',      label: 'Memo',      primitives: [...ENTITY_PRIMITIVES], legendDescription: 'Derived computation (createMemo)' };
const effectSchema: Schema    = { name: 'effect',    label: 'Effect',    primitives: [...ENTITY_PRIMITIVES], legendDescription: 'Side effect (createEffect)' };
const datasourceSchema: Schema = { name: 'datasource', label: 'Data Source', primitives: [...ENTITY_PRIMITIVES], legendDescription: 'External data dependency (fetch, import, etc.)' };
...
const crossComponentReadSchema: EdgeSchema = {
  ...
  legendDescription: 'A component reads a signal or memo owned by another component',
};
...
const storeAccessSchema: EdgeSchema = {
  ...
  acceptsTarget: ['store'],
  legendDescription: 'A component or effect accesses a reactive store',
};
...
const datasourceReadSchema: EdgeSchema = {
  ...
  legendDescription: 'A component or effect reads from an external data source',
};
...
const effectDependencySchema: EdgeSchema = {
  ...
  legendDescription: 'An effect or memo depends on reactive state',
};
...
const rendersSchema: EdgeSchema = {
  ...
  legendDescription: 'Parent component renders child component in its JSX',
};
...
function buildCanvas(analysis: SolidAnalysis): DocumentV2 {
  ...
  return {
    ...
    legend: {
      blurb: [
        '# Solid.js Project Summary',
        '',
        'Auto-generated canvas showing the component architecture, reactive data flow, and external dependencies of this Solid.js codebase.',
        '',
        '**Components** contain their local reactive state (signals, memos, effects) as nested children. **Edges** show data flow between nodes — cross-component signal reads, store access, and the component render tree.',
        '',
        'Generated by `scripts/analyze-solidjs.ts` via static analysis of the TypeScript source.',
      ].join('\n'),
    },
  };
}
